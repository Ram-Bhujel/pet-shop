    ...
    <div class="container">
        ...
        <div class="card rounded-0">
            <div class="card-body">
                ...
                <?php 
                $qry = $conn->query("SELECT c.*, p.product_name, i.size, i.price, i.quantity as stock, p.id as pid 
                                     FROM `cart` c 
                            ...
                            <div>
                                ...
                                <div>
                                    <div class="input-group">
                                        ...
                                        <input type="number" class="form-control form-control-sm qty text-center cart-qty" placeholder="" aria-label="Example text with button addon" value="<?php echo $row['quantity'] ?>" aria-describedby="button-addon1" data-id="<?php echo $row['id'] ?>" data-stock="<?php echo $row['stock'] ?>" readonly>
...
<script>
    ...
    function qty_change($type, _this) {
        ...
        var available_stock = parseInt(_this.closest('.cart-item').find('.cart-qty').attr('data-stock'));
        var new_total = 0;

        if ($type == 'minus') {
            qty = qty - 1;
            if (qty < 1) {
                qty = 1; // Ensure the quantity does not go below 1
            }
        } else {
            qty = qty + 1;
            if (qty > available_stock) {
                alert('Stock limit reached! You can only add up to ' + available_stock + ' items.');
                qty = available_stock; // Cap quantity to available stock
            }
        }
